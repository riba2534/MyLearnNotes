题目链接：[克隆图](https://leetcode-cn.com/problems/clone-graph/)
...
用一个哈希表记录每次新生成的节点，利用递归的思维。令 `cloneGraph(Node)` 的值为新的和原图完全一致的对应的 `Node` 节点，则有 `Node` 的邻居节点 `neighbor` 的值也等于 `cloneGraph(neighbor)` ，不断递归即可。
